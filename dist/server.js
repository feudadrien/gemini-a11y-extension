...
function wcagLinks(sc, prefer = "wcag22") {
...
async function runAxeOnPage(page, runOnlyTags) {
...
server.registerTool("scan_url", { description: "Run axe-core on a URL", inputSchema: ScanArgs.pick({ url: true, ruleset: true, level: true, extraTags: true }).shape }, async (args) => {
...
server.registerTool("scan_html", { description: "Run axe-core on raw HTML", inputSchema: ScanArgs.pick({ html: true, ruleset: true, level: true, extraTags: true }).shape }, async (args) => {
    const browser = await puppeteer.launch({ headless: true });
    try {
        const page = await browser.newPage();
...
server.registerTool("scan_batch", {
...
server.registerTool("scan_file", { description: "Run axe-core on a local HTML file", inputSchema: z.object({ path: z.string(), ruleset: z.enum(["wcag22", "wcag21"]).optional(), level: z.enum(["A", "AA", "AAA"]).optional(), extraTags: z.array(z.string()).optional() }).shape }, async (args) => {
    const browser = await puppeteer.launch({ headless: true });
    try {
        const page = await browser.newPage();
